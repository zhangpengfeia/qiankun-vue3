...
//处理路由格式
export function rebuildRouter<T extends any[]>(router: T, path: string = "") {
  if (router.length) {
...
export function filterAsyncRouter(asyncRouterMap: any[], hier: number, systemIndex?: number, moduleIndex?: number): any[] {
  ...
  return asyncRouterMap.reduce((acc, route, index) => {
    ...
    if (route.menuType === "M") {
      // console.log("route.menuType", route);
      route.component = hier === 1 ? markRaw(Layout) : markRaw(Blank);
      ...
      //如果是菜单,则匹配是否是微前端子应用激活路径,是的话则用Blank代替避免报错
      // route.component = loadView(route.component)
      // console.log("route.menuType---route.component---6666", `/src/views/${route.component}.vue`);
      // console.log("route.menuType---route.component", route.component, route);
